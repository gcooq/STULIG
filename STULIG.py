...
FFF = open('aoa.dat','w')
dec_in_channels = 1
batch_size = 16 #you can choose 16,or ...
iter_num = 30
n_input = 276  # embedding size
n_hidden = 256  # vae embeddings
...
l_encoder_embed_input_t = tf.placeholder(dtype=tf.int32,shape=[batch_size,None])
l_encoder_embed_input_l = tf.placeholder(dtype=tf.int32,shape=[batch_size,None])
l_encoder_embed_input_w = tf.placeholder(dtype=tf.int32,shape=[batch_size,None])
u_encoder_embed_input = tf.placeholder(dtype=tf.int32, shape=[batch_size, None])
u_encoder_embed_input_t = tf.placeholder(dtype=tf.int32,shape=[batch_size,None])
u_encoder_embed_input_l = tf.placeholder(dtype=tf.int32,shape=[batch_size,None])
u_encoder_embed_input_w = tf.placeholder(dtype=tf.int32,shape=[batch_size,None])
...
def time_onehot():
    time_to_vec ={time:vec for time,vec in enumerate(time_tra)}
    vec_to_time ={vec:time for time,vec in time_to_vec.items()}
    return time_to_vec,vec_to_time

def lat_extract():
    lat_to_vec = {lat:vec for lat,vec in enumerate(lat_tra)}
    vec_to_lat = {vec:lat for lat,vec in lat_to_vec.items()}
    return lat_to_vec,vec_to_lat

def wei_extract():
    wei_to_vec = {wei: vec for wei, vec in enumerate(wei_tra)}
    vec_to_wei = {vec: wei for wei, vec in wei_to_vec.items()}
    return wei_to_vec, vec_to_wei
...
def getXs():  # =
    fpointvec = open('data/gowalla_user_vector250d_.dat',
                     'r')  # it has used word2vec
    #     table_X={}  #=
    fpointvec1 = open('data/time.dat',
                     'r')
    item = 0
    ...
    for line in fpointvec1.readlines():
        lineArr = line.split()
        if (len(lineArr) < 24):  # delete fist row
            continue  #
        X = list()
        for i in lineArr[1:]:
            X.append(float(i))  #
        if lineArr[0] == '</s>':
            table_X_t['<PAD>'] = X  # dictionary is a string  it is not a int type
        else:
            table_X_t[lineArr[0]] = X
    print "point number item=", item
    return table_X,table_X_t
...
def readtraindata():
    ...
    Train_Size = 8300  # small data size for gowalla 112 user
...
for i_ in range(len(total_Ts)):
    for j_ in range(len(total_Ts[i_])):
        ...
        new_table_X_l[round(total_Ts[i_][j_][2],6)] = round(total_Ts[i_][j_][2],6)
        new_table_X_w[round(total_Ts[i_][j_][3],6)] = round(total_Ts[i_][j_][3],6)
...
new_table_X_l['<GO>'] = 0
# new_table_X_t['<GO>'] = table_X['<GO>']
new_table_X_w['<GO>'] = 0
new_table_X['<EOS>'] = table_X['<EOS>']
# new_table_X_t['<EOS>'] = table_X['<EOS>']
new_table_X_l['<EOS>'] = 0
new_table_X_w['<EOS>'] = 0
new_table_X['<PAD>'] = table_X['<PAD>']
new_table_X_w['<PAD>'] = 0
# new_table_X_t['<PAD>'] = table_X['<PAD>']
new_table_X_l['<PAD>'] = 0
for keys in new_table_X:
    voc_tra.append(keys)
for keys in new_table_X_t:
    time_tra.append(keys)
for keys in new_table_X_l:
    lat_tra.append(keys)
for keys in new_table_X_w:
    wei_tra.append(keys)
...
time_to_vec,vec_to_time = time_onehot()
lat_to_vec ,vec_to_lat = lat_extract()
wei_to_vec , vec_to_wei = wei_extract()
...
for i in range(len(pointT)):
    ...
    for j in range(len(pointT[i])):
        ...
        temp_t.append(time_to_vec[int(pointT[i][j][1])])
        temp_l.append(vec_to_lat[round(pointT[i][j][2],6)])
        temp_w.append(vec_to_wei[round(pointT[i][j][3],6)])



    new_pointT.append(temp)
    new_pointT_t.append(temp_t)
    new_pointT_l.append(temp_l)
    new_pointT_w.append(temp_w)
...
for i in range(len(test_T)):
    ...
    for j in range(len(test_T[i])):
        ...
        temp_t.append(time_to_vec[int(test_T[i][j][1])])
        temp_l.append(vec_to_lat[round(test_T[i][j][2],6)])
        temp_w.append(vec_to_wei[round(test_T[i][j][3],6)])
    new_testT.append(temp)
    new_testT_t.append(temp_t)
    new_testT_l.append(temp_l)
    new_testT_w.append(temp_w)
...
def dic_em_t():
    dic_embeddings_t = list()
    for keys in new_table_X_t:
        dic_embeddings_t.append(new_table_X_t[keys])
    print dic_embeddings_t
    return dic_embeddings_t
dic_embeddings_t = tf.constant(dic_em_t())


def dic_em_l():
    dic_embeddings_l = list()
    for keys in new_table_X_l:
        dic_embeddings_l.append(new_table_X_l[keys])
    return dic_embeddings_l
dic_embeddings_l = tf.constant(dic_em_l())


def dic_em_w():
    dic_embeddings_w = list()
    for keys in new_table_X_w:
        dic_embeddings_w.append(new_table_X_w[keys])
    return dic_embeddings_w
dic_embeddings_w = tf.constant(dic_em_w())
...
def deconv_network(input_, output_dim, reuse, rate, out_act= None):
    ...
    x = tf.contrib.layers.flatten(x)
    # print(x)
    x = tf.layers.dense(x, units=276, activation=tf.nn.sigmoid)
    # print(x)
    output = tf.reshape(x, shape=[-1, 1, 276])
    return output, h
...
# ENCODER PART of VAE
def encoder(l_y,encoder_embed_input,encoder_embed_input_t,encoder_embed_input_l,encoder_embed_input_w,keep_prob=0.5,reuse=False,std_bias=0):
    with tf.variable_scope("encoder",reuse=reuse):
        encoder_input = tf.nn.embedding_lookup(dic_embeddings, encoder_embed_input)
        encoder_input_t = tf.nn.embedding_lookup(dic_embeddings_t, encoder_embed_input_t)
        encoder_input_l = tf.nn.embedding_lookup(dic_embeddings_l, encoder_embed_input_l)
        encoder_input_w = tf.nn.embedding_lookup(dic_embeddings_w, encoder_embed_input_w)
        activation = lrelu
        encoder_input = encoder_input + pos1_
        encoder_input_l = tf.reshape(encoder_input_l, [16, -1, 1])
        encoder_input_w = tf.reshape(encoder_input_w, [16, -1, 1])
        encoder_input_ = tf.concat([encoder_input, encoder_input_t, encoder_input_l, encoder_input_w], 2)
        pse = tf.reshape(encoder_input_, [batch_size, -1])
        # pseudoinputs = tf.nn.all_candidate_sampler(pse,)
        x_ = tf.reshape(encoder_input_, [-1, batch_size * n_input])
        input_ = tf.transpose(encoder_input_, [1, 0, 2])
        X = tf.reshape(input_, shape=[-1, 1,276,1 ])
...
def get_cost_l(l_y,encoder_embed_input,encoder_embed_input_t,encoder_embed_input_l,encoder_embed_input_w,hidden_dim = 64,K_clusters = 15,num_layers=2,z_dim = n_latent,reuse=False):
    input_,x_, z_mean, z_stddev, w_mean,w_stddev = encoder(l_y,encoder_embed_input,encoder_embed_input_t,encoder_embed_input_l,encoder_embed_input_w, keep_prob,reuse)
    ...
    # KL term-------------
    unreshaped = tf.reshape(dec, [-1,  batch_size * 276])
...
def get_cost_u(u_encoder_embed_input,u_encoder_embed_input_t,u_encoder_embed_input_l,u_encoder_embed_input_w):
    ...
    for label in range(label_size):
        ...
        z1,input_,z,cost_l = get_cost_l([y_i]*batch_size,u_encoder_embed_input,u_encoder_embed_input_t,u_encoder_embed_input_l,u_encoder_embed_input_w,reuse=True)
...
in_z,input_,res_z,cost_l=get_cost_l(l_y,l_encoder_embed_input,l_encoder_embed_input_t,l_encoder_embed_input_l,l_encoder_embed_input_w,reuse=False)
cost_u = get_cost_u(u_encoder_embed_input,u_encoder_embed_input_t,u_encoder_embed_input_l,u_encoder_embed_input_w)  # unlabel data
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(iter_num):
            ...
            while step < len(new_trainT) // batch_size:
                ...
                pred_batch, c_pred, op, batch_cost, l_cost, u_cost, c_cost= sess.run(
                    [pred, correct_pred, optimizer, cost, cost_l, cost_u, cost_c],
                    feed_dict={vae_y_u: decode_uy,l_y: batch_y,
                               l_encoder_embed_input: sources_batch,l_encoder_embed_input_t: sources_batch_t, l_encoder_embed_input_l: sources_batch_l,l_encoder_embed_input_w: sources_batch_w,
                               u_encoder_embed_input: un_sources_batch,u_encoder_embed_input_t:un_sources_batch_t,u_encoder_embed_input_l:un_sources_batch_l,u_encoder_embed_input_w:un_sources_batch_w,
    ...
    while step < len(testT) // batch_size:  #
        ...
        c_pred, pred_batch = sess.run([correct_pred, pred],
                                      feed_dict={l_encoder_embed_input: sources_batch, l_y: batch_y,
                                                 l_encoder_embed_input_t: sources_batch_t,
                                                 l_encoder_embed_input_l: sources_batch_l,
                                                 l_encoder_embed_input_w: sources_batch_w,
